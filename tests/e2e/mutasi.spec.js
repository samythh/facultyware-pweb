// Alur mutasi data yang aman & mandiri-bersih: buat permintaan lalu hapus lagi.
const { test, expect } = require('@playwright/test');
const { login } = require('./helpers');

test.describe('Membuat dan menghapus permintaan', () => {
  test('admin dapat membuat permintaan baru lalu menghapusnya', async ({ page }) => {
    ...
    // Permintaan baru muncul di daftar.
    const row = page.locator('tbody tr', { hasText: judul });
    await expect(row).toBeVisible();

    // Bersih-bersih: ambil id dari tautan Detail lalu hapus lewat endpoint.
